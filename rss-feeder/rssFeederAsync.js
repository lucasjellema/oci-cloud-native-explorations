...
function publishMessages(streamId, messages, callback) {
    ...
    messages.forEach(m => { events.push({ "key": new Buffer.from(m.key).toString('base64'), "value": new Buffer.from(m.payload).toString('base64') }) })
    msg =
        {
            "messages": events
...
async function processRSSAsync(rssURL, filename) {
    ...
    const streamId = 'ocid1.stream.oc1.iad.amaaaaaa6sde7caa4mjocclrqlxxi2dtdj7o5aia66zem23hd6f23muer47a' //process.env['function_triggering_stream_ocid'];

    const event = {
        "function-endpoint": "https://ohv2poh4y2rfodhmdvlvff65mm.apigateway.us-ashburn-1.oci.customer-oci.com/fn/persist"
        , "method": "POST"
        , "headers": []
        , "body": JSON.stringify(
            {
        ...
        , "deliveryAfterTimestamp" : ""
        , "conversationIdentifier":""
        , "responseDeliveryEndpoint" :""
        
    }
    publishMessages(streamId, [{ "key": "file-writer", "payload": JSON.stringify(event) }], function (data) {
        console.log("Messages Published to Stream.");
    });


    return { "rssDoc": result, "eventPublished": "result of publishing event" };
}
...
// set function_triggering_stream_ocid='ocid1.stream.oc1.iad.amaaaaaa6sde7caa4mjocclrqlxxi2dtdj7o5aia66zem23hd6f23muer47a' 
